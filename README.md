...
<script>
    ...
    const MasterDictionary = {
        // 1. 唇音類
        "ㄅ": { 
            desc: "【雙唇音】上下唇先閉合，造成阻力後再讓氣流衝出。*注意不要造成爆音", //----(原本內容)
            pos: { x: 15, y: 55 }, //----(新增座標：雙唇)
            loc: "雙唇"            //----(新增部位名稱)
        },
        "ㄆ": { 
            desc: "【雙唇音】上下唇先閉合，造成阻力後再讓氣流衝出。*注意不要造成爆音", 
            pos: { x: 15, y: 55 }, loc: "雙唇" 
        },
        "ㄇ": { 
            desc: "【雙唇音】上下唇先閉合，造成阻力後再讓氣流衝出。*注意不要造成爆音", 
            pos: { x: 15, y: 55 }, loc: "雙唇" 
        ...
        "ㄈ": { 
            desc: "【唇齒音】上齒接觸下唇。<span class='warning-note'>注意：若沒碰到下唇會變成「ㄏ」的模糊音。</span>", 
            pos: { x: 18, y: 50 }, loc: "唇齒" 
        },

        // 2. 舌尖與舌根
        "ㄉ": { 
            desc: "【舌尖中音】利用舌尖位置（牙齦或硬顎前部）及舌面微凸造成阻礙發音。", 
            pos: { x: 30, y: 35 }, loc: "舌尖與上齒齦" 
        },
        "ㄊ": { 
            desc: "【舌尖中音】利用舌尖位置（牙齦或硬顎前部）及舌面為凹造成阻礙發音。", 
            pos: { x: 30, y: 35 }, loc: "舌尖與上齒齦" 
        },
        "ㄋ": { 
            desc: "【鼻音】鼻翼會有震動感，共鳴在鼻腔。", 
            pos: { x: 45, y: 15 }, loc: "鼻腔共鳴" 
        },
        "ㄌ": { 
            desc: "【邊音】鼻腔不震動，氣流從舌頭兩邊流出。", 
            pos: { x: 35, y: 45 }, loc: "舌尖齒齦" 
        },
        "ㄍ": { 
            desc: "【舌根音】舌後根頂住軟顎。<span class='warning-note'>細節：嘴巴需打開約一個指節寬，否則韻母會跑掉。</span>", 
            pos: { x: 55, y: 40 }, loc: "舌根與軟顎" 
        },
        "ㄎ": { 
            desc: "【舌根音】舌後根頂住軟顎。<span class='warning-note'>細節：嘴巴需打開約一個指節寬。</span>", 
            pos: { x: 55, y: 40 }, loc: "舌根與軟顎" 
        },
        "ㄏ": { 
            desc: "【舌根音】舌後根頂住軟顎。<span class='warning-note'>細節：嘴巴需打開約一個指節寬。</span>", 
            pos: { x: 55, y: 45 }, loc: "舌根與軟顎" 
        },

        // 3. 舌面與平翹舌
        "ㄐ": { 
            desc: "【舌面音】舌面與硬軟顎接觸，氣流從舌面中間衝出。", 
            pos: { x: 45, y: 40 }, loc: "舌面與硬顎" 
        },
        "ㄑ": { 
            desc: "【舌面音】舌面與硬軟顎接觸，氣流從舌面中間衝出。", 
            pos: { x: 45, y: 40 }, loc: "舌面與硬顎" 
        ...
        "ㄒ": { 
            desc: "【舌面音】舌面貼住硬顎。<span class='warning-note'>注意：與英文「C」(舌尖往下)不同。</span>", 
            pos: { x: 45, y: 40 }, loc: "舌面與硬顎" 
        },
        "ㄓ": { 
            desc: "【翹舌音】舌尖要往後捲，指向硬顎的位置。", 
            pos: { x: 40, y: 35 }, loc: "舌尖與硬顎" 
        },
        "ㄔ": { 
            desc: "【翹舌音】舌尖要往後捲，指向硬顎的位置。", 
            pos: { x: 40, y: 35 }, loc: "舌尖與硬顎" 
        ...
        "ㄖ": { 
            desc: "【翹舌音】舌尖垂直放，不可碰到牙齒。", 
            pos: { x: 40, y: 35 }, loc: "舌尖與硬顎" 
        },
        "ㄗ": { 
            desc: "【平舌音】舌尖往前下靠近下齒背，舌面不可碰到上齒背。", 
            pos: { x: 28, y: 45 }, loc: "舌尖與下齒背" 
        },
        "ㄘ": { 
            desc: "【平舌音】舌尖往前下靠近下齒背，舌面不可碰到上齒背。", 
            pos: { x: 28, y: 45 }, loc: "舌尖與下齒背" 
        ...
        // 4. 介符
        "ㄧ": { desc: "【介符/齊齒】嘴角往兩旁拉開做出「一」字形，有助於口型擺動。", pos: { x: 40, y: 55 }, loc: "口腔中心" },
        "ㄨ": { desc: "【介符/合口】雙唇收圓向前突。規律：ㄅㄆㄇㄈ不與ㄨ結合。", pos: { x: 12, y: 55 }, loc: "雙唇收圓" },
        "ㄩ": { desc: "【介符/撮口】嘴唇需用力向中間收攏成圓孔狀。發音時嘴部空間與氣流流暢度並重。", pos: { x: 12, y: 55 }, loc: "雙唇撮攏" },

        // 5. 韻母
        "ㄚ": { desc: "【韻母】嘴巴要徹底打開，像要塞進大獅子頭一樣。", pos: { x: 45, y: 70 }, loc: "口腔全開" },
        "ㄛ": { desc: "【單韻母】圓唇口型。注意：ㄅㄆㄇㄈ後應念ㄛ，維持口型純淨。", pos: { x: 45, y: 65 }, loc: "口腔後部" },
        "ㄜ": { desc: "【單韻母】鴨蛋形長橢圓口型，舌面往下拉壓（含湯匙感）。", pos: { x: 45, y: 60 }, loc: "舌面中後" },
        "ㄝ": { desc: "【單韻母】口型要圓滿，確保足夠空間。", pos: { x: 45, y: 55 }, loc: "口腔中心" },
        "ㄢ": { desc: "【前鼻音】氣流在口腔前部與鼻腔產生共鳴。", pos: { x: 40, y: 20 }, loc: "前鼻腔" },
        "ㄤ": { desc: "【後鼻音】舌後根頂住軟顎。共鳴腔在脖子後方與鼻腔後部。", pos: { x: 60, y: 20 }, loc: "後鼻腔" },
        "ㄦ": { desc: "【特殊韻母】舌尖擺到位後快速鬆開，避免舌頭過度勾起。", pos: { x: 38, y: 42 }, loc: "捲舌位置" }
    };
